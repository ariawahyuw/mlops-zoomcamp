...
def calculate_metrics_postgresql(i):
    ...
    quantile_values = result["metrics"][3]["value"]
    
    with psycopg.connect(CONNECTION_STRING_DB, autocommit=True) as conn:
        with conn.cursor() as curr:
            curr.execute(
                "insert into nyc_taxi_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, quantile_values) values (%s, %s, %s, %s, %s)",
                (
